...
function Menu.build(State, F)
    ...
    Keybinds.register("toggle_menu",      Enum.KeyCode.Five, function() W:ToggleVisible() end)
...
-- Public URL for the "Streak" background art. Roblox can't load a web URL as an
-- image directly, so we download it ONCE into a local file and load that via
-- custom-asset — that way ANYONE running the script gets the picture, not just
-- whoever has the PNG sitting in their workspace.
local BG_URL = "https://i.ibb.co/B5sd5MBp/streaktp.png"

-- Make sure `name` exists locally, downloading from `url` if it's missing.
local function ensureFile(name, url)
    ...
    if body and #body >= 200 then
        local ok = pcall(writefile, name, body); return ok and true or false
    end
    return false
end
...
function Logo.getBackground()
    ...
    local getasset = (type(getcustomasset) == "function" and getcustomasset)
        ...
        or (syn and syn.getcustomasset)
    if getasset and type(isfile) == "function" then
        -- Download the public art if it isn't already on disk, so everyone sees it.
...
function Library.new(parent, title, subtitle)
    ...
    winBtn("X",-38,T.bad,function() self:SetVisible(false) end,"Close (5 reopens)")
